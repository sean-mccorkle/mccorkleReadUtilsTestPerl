...
my $config = new Config::Simple($config_file)->get_block('mccorkleReadUtilsTestPerl');
my $ws_url = $config->{"workspace-url"};
my $ws_name = undef;
my $ws_client = new Bio::KBase::workspace::Client($ws_url,token => $token);
my $auth_token = Bio::KBase::AuthToken->new(token => $token, ignore_authrc => 1);
my $ctx = LocalCallContext->new($token, $auth_token->user_id);
$mccorkleReadUtilsTestPerl::mccorkleReadUtilsTestPerlServer::CallContext = $ctx;
my $impl = new mccorkleReadUtilsTestPerl::mccorkleReadUtilsTestPerlImpl();

sub get_ws_name {
    if (!defined($ws_name)) {
        my $suffix = int(time * 1000);
        $ws_name = 'test_mccorkleReadUtilsTestPerl_' . $suffix;
...

{
    ...
    sub provenance {
        my($self) = @_;
        return [{'service' => 'mccorkleReadUtilsTestPerl', 'method' => 'please_never_use_it_in_production', 'method_params' => []}];
    }
    sub authenticated {
        return 1;
    }
